...
. Create three buckets source, target and metadata buckets. The target bucket contains documents +
that will be created after the Function execution step. Don’t add any documents explicitly to the +
target bucket. +
To create a bucket, refer to xref:manage:manage-buckets/create-bucket.adoc[Create a Bucket].

. From the *Couchbase Web Console* > *Buckets* page, click *source* bucket.
